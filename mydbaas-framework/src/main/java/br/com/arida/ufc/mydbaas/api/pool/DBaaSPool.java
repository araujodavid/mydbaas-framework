...
public class DBaaSPool extends AbstractPool<DBaaS> {
	...
	public boolean save(DBaaS resource) {
		...
		try {
			response = SendResquest.postRequest(this.getClient().getServerUrl()+"/resource/dbaas/add", params, "UTF-8");
	...
	public boolean update(DBaaS resource) {
		...
		try {
			response = SendResquest.postRequest(this.getClient().getServerUrl()+"/resource/dbaas/update", params, "UTF-8");
			if (response.getStatusLine().getStatusCode() != 202) {
				EntityUtils.consume(response.getEntity());
				return true;
			}
	...
	public List<Host> getHosts(DBaaS resource) {
		...
		String json = null;
		try {
			response = SendResquest.postRequest(this.getClient().getServerUrl()+"/resource/hosts", params, "UTF-8");
	...
	public List<VirtualMachine> getMachines(DBaaS resource) {
		...
		String json = null;
		try {
			response = SendResquest.postRequest(this.getClient().getServerUrl()+"/resource/machines", params, "UTF-8");
			json = SendResquest.getJsonResult(response);
		} catch (ClientProtocolException e) {
			e.printStackTrace();
		} catch (IOException e) {
