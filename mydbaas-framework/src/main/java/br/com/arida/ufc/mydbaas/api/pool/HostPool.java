...
public class HostPool extends AbstractPool<Host> {
	...
	public boolean save(Host resource) {
		...
		try {
			response = SendResquest.postRequest(this.getClient().getServerUrl()+"/resource/hosts/add", params, "UTF-8");
	...
	public boolean update(Host resource) {
		...
		try {
			response = SendResquest.postRequest(this.getClient().getServerUrl()+"/resource/hosts/update", params, "UTF-8");
			if (response.getStatusLine().getStatusCode() != 202) {
				EntityUtils.consume(response.getEntity());
				return true;
			}
